...
def post_process_nf(sig_nf, nsamples, duration):
    ...
    # Quit if there is not only 1 peak, meaning that it triggers surely on
    # every system activity (because of trigger normalizatin) and not the AES.
    if len(peaks[0]) != 1:
        print("len(peaks[0]) != 1 ; exit(1)")
        exit(1)
    # If there is only one peak => surely AES => print its position and quit
    # (to quickly modify WHAD parameters).
    if len(peaks[0]) == 1:
        print("peaks[0][0].position={:.2f}%".format((peaks[0][0] / len(sig_nf)) * 100))
        exit(0)
