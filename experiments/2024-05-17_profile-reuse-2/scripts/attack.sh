...
readonly NUM_TRACES_LIST=(65536)
...
readonly SWEEP_MODE_START=1060
readonly SWEEP_MODE_STEP=1
readonly SWEEP_MODE_END=1080

# ** Internals

readonly PLOT_PATH_BASE="$(realpath $(dirname $0))/../plots"
readonly LOG_PATH_BASE="$(realpath $(dirname $0))/../logs"
...
function attack() {
    ...
    local plot_path="${PLOT_PATH_BASE}/attack_${metaset}_${comp}_${num_traces}_${pois_algo}_${pois_nb}_${num_traces_attack}.pdf"
    ...
    local attack_cmd='"${SC_SRC}"/attack.py "${custom_dtype}" --log "${plot}" --save-images --norm --dataset-path "${metaset_path}" --start-point "${start_point}" --end-point "${end_point}" --num-traces "${num_traces_attack}" "${bruteforce}" \
    ...
    if [[ "${SWEEP_MODE_EN}" -eq 0 ]]; then
        ...
        mv "attack_alignment.pdf" "${plot_path}"
        clear
